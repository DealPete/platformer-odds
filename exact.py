...
@lru_cache(maxsize = None)
def playerA_win_prob(ahp, bhp, turn):
    if ahp <= 0: return 0
    if bhp <= 0: return 1
    n_a = a['attack']
    n_b = b['attack']
    ...
    for i in range(a['attack']):
        chance_if_A_hit += B(i + 1, n_a, p_a) * playerA_win_prob(ahp, bhp - i - 1, TURN_B)
    chance_if_B_hit = Fraction(0, 1)
    for i in range(b['attack']):
        chance_if_B_hit += B(i + 1, n_b, p_b) * playerA_win_prob(ahp - i - 1, bhp, TURN_A)
    if turn == TURN_A:
...
chance = float(playerA_win_prob(a['health'], b['health'], TURN_A))
